...
export const homeForRole = (role: UserRole | string | null) => isProductionOperator(role) ? "/production" : isInstaller(role) ? "/profile" : "/"
...
export function canOpenRoute(role: string | null | undefined, path: string) {
    ...
    if (isInstaller(role)) return path === "/profile" || path === "/reset-password" || path === "/login" || path === "/forgot-password"
