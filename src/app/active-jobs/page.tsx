...
export default function ActiveJobsPage() {
    ...
    const { data: result, isLoading } = useSupabaseQuery<{ items: JobRow[]; count: number }>(key, async () => {
        let query = supabase
            ...
            .in("status", ["in_progress", "complete", "invoiced"])
    ...
    return (
        <DashboardLayout>
            <PageShell>
                <PageHeader icon={ClipboardList} kicker="Job & Project Management" title="Active Jobs"
                    description="Live work that has moved beyond quoting — job cards, actuals, completion and invoicing." />
                <div className="flex items-center gap-2 mb-4">
                    <div className="relative flex-1 max-w-md">
                        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" />
                        <Input placeholder="Search job, reference or invoice #…" value={search}
                        ...
                        <div className="border border-border/60 rounded-lg overflow-hidden">
                            <table className="w-full text-sm">
                                <thead className="bg-muted/40 text-muted-foreground"><tr className="text-left">
                                    <th className="font-medium px-4 py-2.5">Job</th><th className="font-medium px-4 py-2.5">Client / Site</th>
                                    <th className="font-medium px-4 py-2.5 w-32">Job number</th><th className="font-medium px-4 py-2.5 w-28">Status</th>
                                </tr></thead>
                                <tbody>{jobs.map(job => {
                                    const meta = STATUS[job.status as keyof typeof STATUS] || STATUS.in_progress
                                    return <tr key={job.id} onClick={() => router.push(`/quoting/${job.id}`)} className="border-t border-border/60 cursor-pointer hover:bg-muted/30">
                                        ...
                                        <td className="px-4 py-3 font-medium tabular-nums">{job.job_number || job.xero_invoice_number || "—"}</td>
                ...
                ) : <div className="py-16 text-center border border-dashed border-border/60 rounded-lg text-sm text-muted-foreground">No active jobs yet. Accepted quotes will move here once they become live jobs.</div>}
            </PageShell>
        </DashboardLayout>
    )
}
