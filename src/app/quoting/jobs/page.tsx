...
function formatDate(value?: string | null) { if (!value) return "—"; return new Date(`${value}T00:00:00`).toLocaleDateString("en-NZ", { day: "numeric", month: "short", year: "numeric" }) }
...
export default function ActiveJobsPage() {
    ...
    const { data: result, isLoading } = useSupabaseQuery<{ items: JobRow[]; count: number }>(key, async () => {
        let query = supabase.from("costing_jobs").select(`*, clients ( name ), stores ( name )`, { count: "exact" }).eq("is_template", false).in("status", ["in_progress", "complete", "invoiced", "cancelled"])
        if (clientId) query = query.eq("client_id", clientId)
        if (search.trim()) { const term = search.trim().replace(/[,()*%]/g, ""); query = query.or(`title.ilike.%${term}%,reference.ilike.%${term}%,job_number.ilike.%${term}%,xero_invoice_number.ilike.%${term}%`) }
        query = query.order("created_at", { ascending: false })
        const from = (page - 1) * PAGE_SIZE; query = query.range(from, from + PAGE_SIZE - 1)
        const { data, error, count } = await query; if (error) throw error
        return { data: { items: (data as JobRow[]) || [], count: count ?? 0 }, error: null }
    })
    const jobs = result?.items || []; const totalCount = result?.count ?? 0
    return <DashboardLayout><PageShell>
        ...
        {isLoading ? <div className="space-y-2">{[1,2,3,4].map(i => <div key={i} className="h-14 rounded-lg bg-muted/40 animate-pulse" />)}</div> : jobs.length ? <><div className="border border-border/60 rounded-lg overflow-hidden"><table className="w-full text-sm"><thead className="bg-muted/40 text-muted-foreground"><tr className="text-left"><th className="font-medium px-4 py-2.5">Job</th><th className="font-medium px-4 py-2.5">Client / Site</th><th className="font-medium px-4 py-2.5 w-32">Job #</th><th className="font-medium px-4 py-2.5 w-36">Complete by</th><th className="font-medium px-4 py-2.5 w-32">Status</th></tr></thead><tbody>{jobs.map(job => { const meta = STATUS[job.status as keyof typeof STATUS] || STATUS.in_progress; return <tr key={job.id} onClick={() => router.push(`/quoting/jobs/${job.id}`)} className="border-t border-border/60 cursor-pointer hover:bg-muted/30"><td className="px-4 py-3"><div className="font-medium">{job.title}</div>{job.reference && <div className="text-xs text-muted-foreground">{job.reference}</div>}</td><td className="px-4 py-3 text-muted-foreground">{job.clients?.name || "Ad-hoc"}{job.stores?.name ? ` · ${job.stores.name}` : ""}</td><td className="px-4 py-3 tabular-nums">{job.job_number || job.xero_invoice_number || "—"}</td><td className="px-4 py-3 tabular-nums">{formatDate(job.completion_date)}</td><td className="px-4 py-3"><Badge variant="secondary" className={meta.className}>{meta.label}</Badge></td></tr> })}</tbody></table></div><TablePagination page={page} pageCount={Math.ceil(totalCount / PAGE_SIZE)} onPageChange={setPage} totalItems={totalCount} pageSize={PAGE_SIZE} /></> : <div className="py-16 text-center border border-dashed border-border/60 rounded-lg text-sm text-muted-foreground">No active jobs yet.</div>}
