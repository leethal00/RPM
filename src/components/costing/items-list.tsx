...
import { Plus, Trash2, ChevronRight, Package2, Search, ArrowUp, ArrowDown, GripVertical, Copy, FileText } from "lucide-react"
                            ...
                            {rows.map(({ it, unitCost, unitSell: us, totalSell }, rowIndex) => {
                                ...
                                const jobCardLink = build && !job.is_template ? (
                                    <Link href={`/quoting/${job.id}/job-card?item=${it.id}`} target="_blank" rel="noopener noreferrer"
                                        ...
                                        aria-label={`Open job card for ${it.name || "item"}`}>
